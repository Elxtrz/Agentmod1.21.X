...
public class MobJailBlockLevel1 extends Block {

    public static final BooleanProperty TRIGGERED = BooleanProperty.of("triggered");
    public static final IntProperty DIA_COUNT = IntProperty.of("dia_count", 0, 64);

    private final ArrayList<EntityType<?>> storedMobs = new ArrayList<>();

    public MobJailBlockLevel1(Settings settings) {
        super(settings);
    ...
    protected void appendProperties(StateManager.Builder<Block, BlockState> builder) {
        builder.add(TRIGGERED, DIA_COUNT);
    }
        ...
        if (!world.isClient()) {
            ...
            if (entity instanceof ItemEntity itemEntity) {
                if (itemEntity.getStack().isOf(Items.DIAMOND)) {
                    ...
                    if (state.get(DIA_COUNT) + (stackAmount) >= 64) {
                        world.setBlockState(pos, ModBlocks.MOB_JAIL_BLOCK_LEVEL_2.getDefaultState());
                        return;
                    }
                }
            }
